...
@app.on_message(command("sreklam") & filters.user(SUDO_USERS))
...
@app.on_message(command("reklam") & filters.user(SUDO_USERS) & ~filters.edited)
async def broadcast_message(_, message):
    if len(message.command) < 2:
    ...
    m = await message.reply_text(
        f"Devam eden yayın,  {len(chats) * sleep_time} Saniye."
    )
    for i in chats:
        try:
            ...
            pass
    await m.edit(f"✈️ **Yayınlanan mesaj {sent} Grup.**")
